...
<section class="container">
    <h4 class="center pink-text bold">Dobrodosli na iTunes Favorites</h4>

    <form action="<?php echo $_SERVER['PHP_SELF'] ?>" class="white form" method="POST">
        <label class="pink-text" for="username"><b>Korisnicko ime:<b></label>
        <input type="text" name="username" value="<?php echo htmlspecialchars($username) ?>">
        <div class="red-text"><?php echo $errors['username']; ?></div>

        <label class="pink-text" for="password"><b>Sifra:<b></label>
        <input  type="password" name="password" value="<?php echo htmlspecialchars($password) ?>">
        <div class="red-text" style="padding-bottom:10px;"><?php echo $errors['password']; ?></div>

        <div class="center">
            <input type="submit" name="login" value="Login" class="btn red lighten-1 z-depth-4">
        </div>
    </form>
